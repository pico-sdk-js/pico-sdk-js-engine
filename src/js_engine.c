...
static char* _nextResource = NULL;
static char* _currentResource = NULL;
static bool __isFirstLoop = true;
...
{
    if (state == JSENGINE_STATE_REQ_RESTART)
    {
        __isFirstLoop = true;
        __jsengine_state = JSENGINE_STATE_REQ_STOPPING;
    }
    else 
    {
...
void psj_run_js_loop()
{
    ...
    {
        __isFirstLoop = false;
        jerry_char_t *autorun = config_get_autorun();
        if (autorun != NULL)
        {
            psj_set_next_resource(autorun);
        }
    }
